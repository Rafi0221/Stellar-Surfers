...
bool Planet::collisionAhead(QVector3D position, QVector3D direction, float distance, float width) {
    if(this->position.distanceToLine(position, direction) > this->radius*0.9 + width)
        return false;
