...
void QuadTreeNode::split(){
    if(!isLeaf())
        return;
    children[NORTH_WEST] = new QuadTreeNode(face, this->factory, this->relativeRotation, this, relativePosition);
    children[NORTH_EAST] = new QuadTreeNode(face, this->factory, this->relativeRotation, this, relativePosition + QVector2D(scale / 2.0 ,0));
    children[SOUTH_EAST] = new QuadTreeNode(face, this->factory, this->relativeRotation, this, relativePosition + QVector2D(scale / 2.0, scale / 2.0));
    children[SOUTH_WEST] = new QuadTreeNode(face, this->factory, this->relativeRotation, this, relativePosition + QVector2D(0, scale / 2.0));
    for (int dir = 0; dir < 4; dir++) {
...
void QuadTreeNode::render(){
    if(isLeaf()){
        ...
        patch->render(dNorth, dEast, dSouth, dWest);
    }else{
        for(int i = 0; i < 4; i++){
            children[i]->render();
        }
